...
class CurrencySeed extends Command
{
    ...
    public function handle()
    {
        ...
        switch (config('core.currency.driver')) {
            case 'model':
                $model = config('core.currency.drivers.model.class');
            ...
            case 'database':
                $table = config('core.currency.drivers.database.table');
                ...
                break;
            case 'filesystem':
                $path = config('core.currency.drivers.filesystem.path');

                Storage::disk(config('core.currency.drivers.filesystem.disk'))->put($path, json_encode(json_decode($str)));
                break;
        }
    }
}
